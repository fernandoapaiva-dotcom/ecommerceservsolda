    ...
    <div className="space-y-16 pb-16">
                ...
                {banners[currentBannerIdx].link && (
                  <div>
                    <Link
                      ...
                      className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-slate-900 font-bold px-6 py-3 rounded-full text-sm tracking-wide transition-all uppercase shadow-lg hover:shadow-amber-500/20 hover:scale-105"
                    >
                      <span>Ver Mais</span>
                      <ArrowRight size={16} />
                    </Link>
          ...
          {banners.length > 1 && (
            <>
              <button
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 bg-slate-900/60 hover:bg-amber-500 hover:text-slate-900 text-white p-2.5 rounded-full transition-colors hidden sm:block"
              >
                <ArrowLeft size={20} />
              </button>
              <button
                onClick={handleNextBanner}
                className="absolute right-4 top-1/2 -translate-y-1/2 bg-slate-900/60 hover:bg-amber-500 hover:text-slate-900 text-white p-2.5 rounded-full transition-colors hidden sm:block"
              >
                <ArrowRight size={20} />
              </button>
            </>
          ...
          <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2">
            {banners.map((_, idx) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all ${currentBannerIdx === idx ? 'bg-amber-500 w-6' : 'bg-slate-700/80'}`}
              />
            ))}
          </div>
        </div>
        ...
        <div className="relative w-full h-[320px] md:h-[450px] bg-gradient-industrial flex items-center overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative">
            <div className="max-w-2xl text-white space-y-6">
              <span className="bg-amber-500/10 text-amber-500 px-3.5 py-1 rounded-full text-xs font-bold tracking-widest uppercase border border-amber-500/20">
                Líder em Soldagem
              </span>
              <h1 className="text-4xl md:text-6xl font-black tracking-tight leading-none">
                Soldagem de <br /><span className="text-amber-500">Alta Performance</span>
              </h1>
              ...
              <div>
                <Link to="/produtos" className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-slate-900 font-bold px-8 py-3.5 rounded-full transition-all uppercase text-sm tracking-wider">
                  Explorar Catálogo
                </Link>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-slate-200 pb-4">
          ...
          <Link to="/produtos" className="text-sm font-bold text-amber-600 hover:text-amber-700 mt-2 md:mt-0 inline-flex items-center gap-1">
            Ver Todos
          </Link>
        </div>
          ...
          {sections.map(section => (
            ...
            >
              ...
              <div className="absolute bottom-4 left-4 right-4">
                <span className="text-lg font-extrabold text-white group-hover:text-amber-500 transition-colors uppercase tracking-wide">
                  {section.name}
                </span>
              </div>
            </Link>
      ...
      {featuredProducts.length > 0 && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-slate-200 pb-4">
            ...
            <Link to="/produtos" className="text-sm font-bold text-amber-600 hover:text-amber-700 mt-2 md:mt-0">
              Ver Todos Produtos
            </Link>
          </div>
              ...
              return (
                <div key={product.id} className="group bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col">
                  <div className="relative aspect-video bg-slate-100 overflow-hidden">
                    ...
                    />
                    <div className="absolute top-2 left-2 bg-amber-500 text-slate-900 text-[10px] font-black tracking-widest px-2.5 py-1 rounded-full uppercase">
                      Destaque
                    </div>
                  </div>
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                    <div>
                      ...
                      <h3 className="text-base font-bold text-slate-900 line-clamp-1 group-hover:text-amber-600 transition-colors mt-1">
                        {product.name}
                      </h3>
                    ...
                    <div className="flex items-center justify-between border-t border-slate-100 pt-3">
                      ...
                      <Link
                        ...
                        className="bg-slate-900 hover:bg-amber-500 text-white hover:text-slate-900 p-2 rounded-xl transition-colors"
                      >
                        <Eye size={18} />
                      </Link>
                    </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
          <div className="lg:col-span-2 rounded-2xl overflow-hidden shadow-md border border-slate-200 h-80 lg:h-auto min-h-[320px]">
            <iframe
              title="Localização ServoSolda"
              src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3483.9213456788544!2d-51.1783456849042!3d-29.167345682206775!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x951ea32a321d321d%3A0x321d321d321d321d!2sCaxias%20do%20Sul%2C%20RS!5e0!3m2!1spt-BR!2sbr!4v1624632145678!5m2!1spt-BR!2sbr"
              width="100%"
              height="100%"
              style={{ border: 0 }}
              allowFullScreen=""
          ...
          <div className="bg-slate-900 text-white p-8 rounded-2xl flex flex-col justify-between space-y-6">
            <div>
              <h3 className="text-xl font-bold uppercase tracking-wider text-amber-500">ServoSolda Showroom</h3>
